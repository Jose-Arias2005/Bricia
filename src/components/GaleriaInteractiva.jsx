...
export default function GaleriaInteractiva() {
  // Datos actualizados con iconos temáticos
  ...
  // La imagen principal actual es siempre la primera del array rotado
  ...
  return (
    <section className="relative">
      ...
      {/* Contenedor Principal del Carrusel con Botones Laterales */}
              ...
              return (
                ...
                >
                  <motion.img 
                    src={item.img} 
                    alt={item.title}
                    // Solo aplicamos layoutId a la imagen si NO es la que se va a abrir en el lightbox
                    // Esto evita conflictos visuales durante la apertura
                    layoutId={selectedId !== item.id ? `img-${item.id}` : undefined}
                    className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" 
                  />
                  
                  {/* Overlay "Fresco" con gradiente azul */}
      ...
      <AnimatePresence>
        {selectedId && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={() => setSelectedId(null)} // Cerrar al hacer click en el fondo
          ...
          >
            ...
            {items.map(item => item.id === selectedId && (
              <motion.div 
                key={item.id}
                layoutId={`card-${item.id}`} // ¡Magia! Esto conecta la miniatura con el modal
                className="relative max-w-5xl w-full max-h-full rounded-2xl overflow-hidden shadow-2xl aspect-video bg-slate-900"
                onClick={(e) => e.stopPropagation()} // Evitar que clicks en la imagen cierren el modal
              >
                 <motion.img 
                    src={item.img} 
                    alt={item.title}
                    layoutId={`img-${item.id}`} // Conecta la imagen interna
                    className="w-full h-full object-contain" 
                  />
                  {/* Detalles en el lightbox */}
